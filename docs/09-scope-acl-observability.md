...
- Executions run under a **system context** (`AppArea` = crontab-like). Recording the authoring admin on the definition for audit is **planned, not implemented** — `mageos_workflow` has no author column today ([#14](https://github.com/mage-os-lab/magento-workflows/issues/14)); definition saves are logged, but the log line is the only authorship trail.
- Definition saves are logged (plays well with admin-activity modules).
- Scope is re-checked at *execution* time, not just authoring time — a workflow scoped to website 1 whose author lost website-1 access gets suspended, not silently escalated ([Security §Deferred privilege escalation](10-security.md#deferred-privilege-escalation-the-core-threat-model)). **Not implemented** ([#14](https://github.com/mage-os-lab/magento-workflows/issues/14)); the dispatcher's website check is on the *entity's* scope, not the author's access.
