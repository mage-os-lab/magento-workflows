...
- **Where the resources are declared matters as much as what they gate.** Magento denies an ACL resource no installed module has declared, so a resource referenced from `mage-os/workflows`' own `etc/webapi.xml` or validation pipeline has to be declared by `mage-os/workflows` — otherwise a headless install (engine + REST, no admin UI) 403s on every route. Core therefore owns and titles the tree root `MageOS_Workflows::workflows` plus `::view`, `::manage`, `::dry_run`, `::secrets` and the per-action-group `::action_*` gates. `mage-os/workflows-admin-ui` declares only `::enable` and `::manual_run` — the two whose sole enforcement points are admin controllers — grafting them onto the core-declared root by id, without re-titling it; `mage-os/workflows-approvals` grafts its two the same way. Exactly one package titles each resource, so the merged role tree never depends on module load order. Pinned by `CoreAclCoverageTest`.
...
- Executions run under a **system context** (`AppArea` = crontab-like). Recording the authoring admin on the definition for audit is **planned, not implemented** — `mageos_workflow` has no author column today ([#14](https://github.com/rhoerr/magento-workflows/issues/14)); definition saves are logged, but the log line is the only authorship trail.
- Definition saves are logged (plays well with admin-activity modules).
- Scope is re-checked at *execution* time, not just authoring time — a workflow scoped to website 1 whose author lost website-1 access gets suspended, not silently escalated ([Security §Deferred privilege escalation](10-security.md#deferred-privilege-escalation-the-core-threat-model)). **Not implemented** ([#14](https://github.com/rhoerr/magento-workflows/issues/14)); the dispatcher's website check is on the *entity's* scope, not the author's access.
- **Approval tasks split viewing from deciding** ([Approval Gate discovery §5](discovery/approval-gate.md#5-rest-api-the-post-a-decision-and-payload-surface)): the optional `mage-os/workflows-approvals` addon declares `MageOS_WorkflowsApprovals::approvals_view` and `MageOS_WorkflowsApprovals::approvals_decide` (grafted onto the `MageOS_Workflows::workflows` tree by id — the addon owns and titles these two, not core). Deciding is deliberately **not** implied by `::manage` — the people who approve refunds are usually not the people who author workflows — and `::approvals_view` stays separate from the execution grid's `::view` because approval tasks carry interpolated PII (title/instructions snapshots) the execution grid doesn't surface as prominently. If a gate declares `assignee_role`, deciding additionally requires the actor to hold that role, enforced at decide time (not just filtered in the grid).
...
- **ES indexing** (planned, phase 2 — [#16](https://github.com/rhoerr/magento-workflows/issues/16)): reuse the async-events ES indexing hook to index execution records for the same Lucene querying. Indexing is redaction-by-default — metadata + IDs; full payloads opt-in ([Security §PII containment](10-security.md#pii-containment)). No execution indexing exists today, and the redaction layer must ship with it rather than after.
