...
`schema` accepts `1`, `2`, `3`, or `4`. Version 2 adds exactly one step type (`wait`) and two optional delay fields (`business_days`, `at`); version 3 adds exactly one step type (`switch`); version 4 adds exactly one step type (`approval`). Nothing else changes at any bump, and older documents remain valid unchanged. The compat rule is enforced, not advisory: a document using any v2 feature **must** declare `"schema": 2` or later, a document using `switch` **must** declare `"schema": 3`, and a document using `approval` **must** declare `"schema": 4` — the parser (`Model/Definition/Definition.php`) rejects them below the required version, as does the published JSON Schema.
