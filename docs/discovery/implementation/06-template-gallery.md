...
| Parameter engine | `Model/Template/` | `%param.key%` token substitution (deliberately distinct from runtime `{{ }}`); typed params → form fields rendering from the **recorded F6 option-source union** (inline `options` vs `options_search` via `meta/options` — `entity:salesrule` etc. are search-typed sources registered in the option-source pool), plus `duration` and `secret` pick-or-create widgets; leftover tokens = hard error before any write. If 06 lands before 07, this stage builds the `meta/options` endpoint (first-consumer rule) |
