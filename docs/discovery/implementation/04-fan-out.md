...
the notifier consumer, producing N ordinary single-entity executions. The dispatcher and
executor are unchanged except for accepting the pre-built `origin` payload key. The mid-flow
`fan_out` step (discovery F2) is explicitly out of this plan — it gets its own plan if/when F1
demand proves out, and would ride the next schema revision.
...
| `FanOutExpander` | `Model/Engine/` (core), invoked from `WorkflowNotifier` (triggers-core) | Lives at the notifier because that's where the source event object — and its trace UUID — exists. Resolves the relation (F5, cardinality `many`) from the source snapshot, hydrates each target via the existing hydrators into the child trigger snapshot, injects `origin` (`{event, entity_type, entity_id, trace_uuid, via: "fan_out"}`), dispatches per target. Cap: truncate + warn + admin-visible marker (never silent) |
...
| # | Stage | Notes |
|---|---|---|
| 1 | Column + expander + notifier wiring + type-alignment check + storm/debounce tests | Core behavior, inert until a workflow sets `fan_out` |
| 2 | `origin` payload convention + `origin_uuid` column + grid filter | Convention documented in docs/04 per F8 |
| 3 | UI fieldset + plain language + confirmation copy | |
