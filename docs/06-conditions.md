...
- **Five roots**: `sales_order`, `customer`, `quote`, `catalog_product`, `newsletter_subscriber`. Quote is a first-class root (`Condition\Quote\Attribute` + `Combine`, `QuoteHydrator` via `CartRepositoryInterface`) — "cart total > $100" on `quote.abandoned` needs no workaround. `newsletter_subscriber` is contributed by the optional `workflows-newsletter` domain pack (`Condition\Subscriber\Attribute`/`Combine`) — guests subscribe without a customer account, so it's a root in its own right rather than a customer attribute.
