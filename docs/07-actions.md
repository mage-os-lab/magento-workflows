...
| Customer | assign group · set custom attribute · subscribe/unsubscribe newsletter · anonymize⁴ |
| Catalog | set attribute value (scoped)¹¹ · enable/disable product¹¹ · set stock status/qty (MSI source-aware⁵) · set categories (add/remove/replace) · set special price (from/to dates; `clear: true` removes it)¹¹ · assign product websites (add/remove/replace)⁸ · set product links (related/cross-sell/up-sell, add/remove/replace per link type)⁹ |
| Review | set review status (approve/pending/not-approve)¹⁰ |
| Marketing | generate coupon from cart price rule⁷ |
...
⁸ `product.assign_websites` computes `add`/`remove` target sets from the product's *current* website membership (idempotent under redelivery, same shape as `product.set_categories`); `replace` overwrites. Applied in one `ProductRepositoryInterface::save`, so the whole target set lands atomically. Every configured website id is validated up front against `WebsiteRepositoryInterface` — an unknown id is a terminal step failure, not a silent skip.

⁹ `product.set_product_links` adds/removes/replaces one link type (`related` | `crosssell` | `upsell`) per step; `add`/`remove` compute the target set from that type's current links (idempotent under redelivery). Because the underlying API replaces the product's whole link collection in one call, the other two link types are read back and re-submitted unchanged so editing related links can't wipe cross-sells/up-sells. A target SKU that doesn't exist, or a self-link, is skipped with a warning in the step output rather than failing the whole step; a run that changes nothing and skips nothing is itself a no-op (`skipped`).

¹⁰ `review.set_status` sets a review's moderation status (approve/pending/not-approve) and is only meaningful on a workflow started by the `Review Submitted` or `Review Status Changed` trigger — the review is resolved from the trigger context (`review_id`), not from the `catalog_product` entity the step otherwise runs against. Authoring this action is gated by the `MageOS_Workflows::action_review` ACL resource, alongside the other per-group `action_*` gates ([Scope & ACL §ACL](09-scope-acl-observability.md#acl)).

¹¹ **Trigger-chaining boundary:** these three actions write through `Magento\Catalog\Model\Product\Action::updateAttributes()` (a deliberate choice — only the touched attribute is reindexed, no full product save, no plugin storm). That path does not fire `catalog_product_save_after`, so a price or status change made **by a workflow** does not raise `catalog.product.price_changed` / `catalog.product.status_changed` and will not chain into workflows listening for those triggers. A price/status change made through the admin product form, the API, or an import fires them normally. If a chained reaction is required, have the first workflow do the follow-up work itself (its own next steps see the change), or use a webhook.
...
- **Chain depth:** executions carry `chain_depth`, propagated through a process-local `ChainDepthContext`: the executor brackets its walk with it, `EventPublisher` stamps the depth onto any payload published while it is active (as a reserved ride-along key the notifier strips before the payload becomes a trigger snapshot), and the notifier feeds it to every dispatch — exceeding `loop_guard_depth` (default 1) skips dispatch and logs `loop_suppressed`. Coverage boundary: only events flowing through this suite's `EventPublisher` carry depth; upstream async-events published through the queue publisher are delivered in another process and dispatch at depth 0, where the debounce window and circuit breaker remain the guards.
