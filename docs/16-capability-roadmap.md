...
**Status:** In execution. This plan operationalizes the July 2026 state-and-outlook review.
...
|---|---|---|
| `inventory.stock_threshold_crossed` publisher | Trigger declared in `workflow_triggers.xml` + `async_events.xml` but nothing publishes it | New `StockThresholdDetector` cron in `workflows-scheduler` (legacy stock, hysteresis flag table, batch-capped) |
| Quote schedule queries | `quote` entity had no repository wired in `QueryRunner` | Wire `CartRepositoryInterface` in scheduler `di.xml` |
...
- **Generic Trigger Data condition**: leaf condition matching any dot-path in the trigger payload (e.g. `status_from` / `status_to` on `sales.order.status_changed`). Available under every combine.
...
Definition schema bumps to **version 2** (`schema: 1|2` accepted; v1 documents remain valid — v2 only adds optional fields and one step type).
...
### Wave 6 — Test coverage (continuous through all waves)
