...
- Steps are marked `running` with a claim timestamp so a sweeper can fail-or-retry zombies. The step row is one row per `(execution_id, step_key)` — a unique key, written with `INSERT … ON DUPLICATE KEY UPDATE` over exactly the columns the caller supplied (never the ones it didn't, so a re-park cannot wipe a recorded decision `result`). The old SELECT-then-INSERT was the same racy shape the debounce guard rejects: two consumers holding one execution could both insert, and a `LIMIT 1` reader would then route on whichever duplicate came back first.
- **Nothing stays `running` forever.** The zombie sweep matches step rows stuck `running`; a second pass in the same cron matches *executions* stuck `running` whose step rows hold nothing `running` or `pending` — the crash window where the step row was already closed but the execution row had not moved on. Both republish to the execute topic, which is safe precisely because of the resume-past-completed-step guard below. Staleness for both is the same 30-minute clock, measured for the execution pass against the newest of its step timestamps (or `triggered_at` when it has none), so a live consumer between two steps is never stolen. See [Operations §Required infrastructure](15-operations.md#required-infrastructure).
- **A suspended workflow stops mid-flight.** If the circuit breaker suspended the workflow while this execution was queued or parked, the executor fails the execution terminally instead of walking it ([Actions §Guards](07-actions.md#loop-prevention-storms-and-circuit-breaking)). A workflow *deleted* mid-flight is different: it still executes from its pinned snapshot.
- **A step that already finished is resumed past, not re-run.** `current_step` only advances at the top of the *next* step's iteration, so an execution whose step completed still points at that step until the following pre-write lands — and every redelivery path re-enters the walk there (a retryable failure thrown by a later step, a zombie republish, a consumer death between the step-row write and the execution-row save). Each loop iteration therefore reads the step row *before* claiming it `running`: a `complete` action row is skipped to its static `next` edge (rehydrating the recorded output into the context bag, which closes the window where the row landed but the context save did not); a `complete` branch/switch row follows the edge its recorded `{result}` / `{matched}` originally took, so a resumed walk cannot diverge because the entity changed underneath it (an unreadable result falls back to re-evaluating — conditions are side-effect free); a `complete` stop row completes the execution. Rows in `running`/`pending`/`failed` are re-executed — that is what at-least-once means. Park steps (delay/wait/approval) are deliberately excluded: `current_step` never rests on a completed park step in the normal flow, and `ResumeConsumer` owns their decision routing, so re-parking beats duplicating those rules in a second place. Pinned by `ExecutorWalkTest` §9.
- Actions should be idempotent where cheap; where not, **at-least-once is documented per action**. The dedupe key is always **execution UUID + step key** — fine enough that two comment steps in one workflow coexist, coarse enough that a redelivered single step still dedupes. The executor guard above is the first line of defence and the per-action dedupe is the second: the guard cannot help a crash *inside* a step, before its row was marked complete. Four guard shapes are in use, each chosen for what the side effect makes cheap:
  - **Marker written with the effect** — `order.add_comment` (invisible HTML-comment marker in the comment, `AddCommentTest`) and `order.create_creditmemo` (the same marker on a memo comment, written inside the refund's own transaction, `CreateCreditmemoDedupeTest`).
  - **Natural idempotence** — `order.add_tracking` skips when the order already carries the same `(carrier_code, track_number)`; the pair IS the parcel's identity, so no marker is needed (`AddTrackingTest`).
  - **Derived identity** — `marketing.generate_coupon` derives the coupon CODE from the dedupe key, so the redelivery re-derives it, finds the coupon and returns the same code; `UNIQUE(code)` settles any race (`GenerateCouponTest`).
  - **Durable claim before the effect** — `notify.email` / `order.send_email` INSERT a claim into `mageos_workflow_send_log` (`UNIQUE(claim_key)`) BEFORE the send; a duplicate key means already claimed, so the step skips. Mail can be neither undone nor detected afterwards, so this one is deliberately at-MOST-once: a crash between claiming and confirming leaves a `claimed` row and the redelivery skips a message that may never have gone out, saying so in the skip reason. (This replaced a cache check-and-set, which was non-atomic, per-node on file caches, and erased by `cache:flush`.)
