...
- Actions should be idempotent where cheap; where not, **at-least-once is documented per action**. The dedupe key is always **execution UUID + step key** — fine enough that two comment steps in one workflow coexist, coarse enough that a redelivered single step still dedupes (add-comment pinned by `AddCommentTest`; credit memos carry the same marker on a memo comment written in the refund's own transaction, pinned by `CreateCreditmemoDedupeTest`; email send logs the key before SMTP). The executor guard above is the first line of defence and the per-action dedupe is the second: the guard cannot help a crash *inside* a step, before its row was marked complete.
